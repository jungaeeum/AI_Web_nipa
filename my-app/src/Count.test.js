import { render, screen, fireEvent } from "@testing-library/react"; //fireEvent : 이벤트 발생 테스트를 위한 객체
import App from "./App02";

// 최초 렌더링 시 상대값이 0으로 표시되는지 테스트하기
...
test("When the + button is pressed, the counter changes to 1", () => {
  ...
  fireEvent.click(buttonElement); // fireEvent 객체가 클릭 메소드에 요소를 전달받아 클릭 이벤트를 실험(?)한다
  const counterElement = screen.getByTestId("counter");
